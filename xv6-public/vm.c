...
seginit(void)
{
  ...
  // DPL，即描述符特权级，CPU给出的虚拟地址会先经过段翻译成线性地址，这时有一次权限检查，
  // 比如检查代码段选择子cs的CPL与代码段的DPL，仅当CPL<=DPL时，才通过检查，表明此时CPU所处
  // 的模式有权限访问内存。之后再页翻译，也会检查权限。
  c = &cpus[cpuid()];
...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  a = PGROUNDUP(oldsz); // 从PGROUNDUP(oldsz)开始到newsz。
