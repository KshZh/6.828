...
block_is_free(uint32_t blockno)
{
	if (super == 0 || blockno >= super->s_nblocks)
		return 0;
    // blockno/32获得uint32_t数组中blockno对应的那个位所处的uint32_t，
    // blockno%32获得blockno对应的那个位在那个uint32_t中的偏移量。
...
dir_alloc_file(struct File *dir, struct File **file)
{
	...
	for (i = 0; i < nblock; i++) {
		...
		f = (struct File*) blk;
		for (j = 0; j < BLKFILES; j++)
            // 在目录文件中找到一个free的File对象可被分配。
			if (f[j].f_name[0] == '\0') {
				*file = &f[j];
				return 0;
			}
	}
    // 目录文件没有足够空间，那就再分配一个block。当然分配和载入的事由bc_pgfault处理。
	dir->f_size += BLKSIZE;
	if ((r = file_get_block(dir, i, &blk)) < 0)
		return r;
	f = (struct File*) blk;
...
{
    // JOS中创建文件不用创建inode，只需要在其所在目录文件中分配一个File对象。
	char name[MAXNAMELEN];
	int r;
	struct File *dir, *f;
